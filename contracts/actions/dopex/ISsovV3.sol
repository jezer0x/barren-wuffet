...
interface ISsovV3 is IERC721Enumerable {
    ...
    function getCheckpoints(uint256 epoch, uint256 strike) external view returns (VaultCheckpoint[] memory);
    ...
    function calculateSettlementFees(uint256 pnl) external view returns (uint256);

    function getEpochTimes(uint256 epoch) external view returns (uint256 start, uint256 end);

    function getEpochStrikes(uint256 epoch) external view returns (uint256[] memory);
