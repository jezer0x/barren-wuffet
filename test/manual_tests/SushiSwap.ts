...
async function main() {
  ...
  console.log(ethers.utils.formatEther(prevEthBalance));
  await McFund.takeAction(
    trueTrigger,
    {
      callee: sushiSwapExactXForY.address,
    ...
    [await usdc_contract.balanceOf(McFundAddr)],
    [BigNumber.from(0)] // 0 fees set in deploy
  );
  let postEthBalance = await ethers.provider.getBalance(McFundAddr);
  console.log(ethers.utils.formatEther(postEthBalance));
  console.log(ethers.utils.formatEther(postEthBalance.sub(prevEthBalance)));
  balance_usdc = await usdc_contract.balanceOf(McFundAddr);
  console.log(balance_usdc.toString());
